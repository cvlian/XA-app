 ...
 * @version $Id: svr_stat_v1.h 108167 2015-08-04 06:11:25Z eugene $
 */
...
#define TBSTAT_ERR_INVALID_TID              (TBSTAT_ERR_BASE -    4)
...
struct tbstat_info_s {
    int thr_cnt;
    /* wthr 는 wtid_start부터 wtid_end까지 순차적으로 존재
       wthr 갯수 = wtid_end - wtid_start + 1 */
    int wtid_start;             /* working thread 의 ID 첫 번호 */
    int wtid_end;               /* working thread 의 ID 끝 번호 */
...
typedef struct tbstat_info_s tbstat_info_t ;
 ...
 /*========================================================================
  * struct cur_wait_t:
  * thr_get_current() 에서 현재 wait하고 있는 세션인 경우
 ...
 * @return      error code
 *                  - TBSTAT_ERR_SUCCESS
 *                  - TBSTAT_ERR_INVALID_TID: 인자로 들어온 TID가 working thr의
 *                                     tid가 아니거나 주어진 thread 가
 *                                     active 하지 않을 때
 */
#ifdef _WIN32
__declspec(dllexport)
int
thr_get_current(int tid, stat_history_elem_t* curr, cur_wait_t *cw);
#else
int
thr_get_current(int tid, stat_history_elem_t* curr, cur_wait_t *cw);
#endif
 ...
 *
 * @param[in]   wthr_id: WTHR tid
 *              elem_id: stat history의 element ID (0~SQL_STAT_HISTORY_QSIZE-1)
 * @param[out]  e: (wthr_id, elem_id)에 해당하는 stat history element 주소
 *
 * @return      error code
 *                  - TBSTAT_ERR_SUCCESS
 *                  - TBSTAT_ERR_INVALID_TID: 인자로 들어온 TID가 working thr의
...
__declspec(dllexport)
int
get_stat_history_elem(int wthr_id, int elem_id, stat_history_elem_t **e);
#else
int
get_stat_history_elem(int wthr_id, int elem_id, stat_history_elem_t **e);
#endif
